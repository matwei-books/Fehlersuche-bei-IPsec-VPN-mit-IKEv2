...
und kann andererseits für Angriffe auf die Infrastruktur dienen.
Bereits in :cite:`Weidner201703`
hatte ich einiges zur Behandlung von ICMP bei Firewalls geschrieben
und will das hier nicht weiter als nötig ausdehnen.

RFC4301 :cite:`RFC4301` teilt ICMP-Traffic in die beiden Kategorien
Fehlermeldungen (error), wie zum Beispiel *ICMP Unreachable*,
und informative Meldungen (non-error), wie zum Beispiel PING,
ein.

Informative ICMP-Nachrichten sollen über die Policy,
das heißt wie normaler Traffic, behandelt werden
und entsprechend der SPD weitergeleitet oder verworfen werden.
Nötigenfalls werden neue Child-SA ausgehandelt,
wenn noch keine existieren
...
oder es sich um einen ausgeklügelten Angrif handelt.
...
dem Administrator die Möglichkeit zu geben,
...
sind im Normalfall vertraunswürdiger,
weil ihre Herkunft und Beschaffenheit einfacher überprüft werden kann,
wenn man die Kontrolle über das Netz hat
und Missbrauch entgegen wirken kann.

Trotzdem sollten Steuerungsmöglichkeiten für den Empfang dieser
Nachrichten am Gateway vorhanden sein.
...
die geschützt durch das VPN zwischen den roten Netzen übertragen werden.
...
Kommt ICMP-Traffic für das Netz beim Peer auf der roten Seite des
VPN-Gateways an
...
Eine entsprechende Child-SA wird nötigenfalls eingerichtet,
...
Dass VPN-Gateway der Gegenstelle,
...
und schreibt bei einer nicht bestandenen Prüfung
ebenfalls einen Logeintrag
...
ist unmittelbar eine synthetisierte ICMP-Nachricht
an den Sender im geschützten Netz senden,
denn dessen großes Datagramm ist bereits verschlüsselt gesendet
...
Das bedeutet bei einer länger bestehenden SA
periodisch das oben beschriebene Spiel wiederholt wird.
